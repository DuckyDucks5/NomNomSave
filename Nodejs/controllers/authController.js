...
exports.verifyEmail = (req,res) => {
  ...
  userModel.setEmailVerified(email, (err) => {
    ...
    res.status(200).send("<h2>Email successfully verified!</h2><p>You may now log in.</p>");
  })
}
//----------------------------------------------
...
exports.resendVerifiyCode = (req,res) => {
  ...
  userModel.findUserEmail(email, (err, result) => {
    ...
    if(user.LastVerificationSentAt && now - user.LastVerificationSentAt < 30000){
      const waitTime = Math.ceil((30000 - (now - user.LastVerificationSentAt))/1000);
      return res.status(429).json({error: `Please wait ${waitTime} seconds before resending.`});
    }
    ...
    userModel.updateResetToken(userId, token, tokenExpiry, now, (err2) => {
      if(err2){
        return res.status(500).json({error: err2.message});
      }
...
exports.resetPassword = (req, res) => {
  ...
  userModel.updatePassword(email, newPassword, (err) => {
      ...
      userModel.updateResetToken(email, null, null, () =>{
        if(err){
          return res.status(500).json({error: err.message});
        }
      })
